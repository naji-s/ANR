...
class BasePU:
    ...
    def _make_propensity_weighted_data(x,s,e,sample_weight=None):
        ...
        if  type(x).__name__ == 'csr_matrix' or type(x).__name__ == 'coo_matrix':
            Xp = sparse_vstack((x,x))
        else:
            Xp = np.vstack((x, x))
        Yp = np.hstack((np.ones_like(s), np.zeros_like(s)))
        Wp = np.hstack((weights_pos, weights_neg))
        return Xp, Yp, Wp
