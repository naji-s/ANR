    ...
    #         tf.print("Keras layer name:", type(self.keras_model.sig_layer).__name__)
        ...
        for var in self.keras_model.trainable_variables:            
            ...
            if 'sig_layer/alpha' in var.name:
                var_list += [ones(tf.shape(self.keras_model.sig_layer.alpha)[0])]
            if 'sig_layer/beta' in var.name:
                var_list += [[1e-2]]
            if 'psychometric_layer/alpha' in var.name:
                var_list += [ones(tf.shape(self.keras_model.psych_layer.alpha)[0])]                
...
#         history_, parameter_reporter, lookback_reduce_lr
    ...
    def keras_fit(self, X_train=None, l_y_decimal_train=None, validation_data=None, lr=None, optimizer=None):
            ...
            try:
                ...
                try:
                    try:
                        ...
                        results=tfp.optimizer.lbfgs_minimize(value_and_gradients_function=lambda x: func_and_grad_generator(x)[:2], initial_position=init_params, max_iterations=self.max_iter, tolerance=1e-8, parallel_iterations=1)
...
#             self.little_input_dict = {'psych_input': tf.convert_to_tensor([input_dict['psych_input'][0]], dtype=tf.float64), 'sig_input': tf.convert_to_tensor([input_dict['sig_input'][0]], dtype=tf.float64), 'idx':[input_dict['idx'][0]]}
    ...
    def fit(self, X, y, validation_data=None, c_estimation_method='elkan', weights=None, reinitialize=True, gp_kernel_setting=None, fresh_opt_initial_point=False):    
        # tf.config.threading.set_inter_op_parallelism_threads(1)
        # tf.config.threading.set_intra_op_parallelism_threads(1)
            ...
            if warm_start:
                if weights is None:
                    if c_estimation_method == 'elkan':
                        warm_start_model = PUAdapterTF(encoder=self.encoder)
...
#                         warm_start_model = KMEModel(encoder=self.encoder)
#                     elif c_estimation_method == 'sar-em':
#                         warm_start_model = PropensityEM(encoder=self.encoder)
#                         if c_estimation_method != 'sar-em':    
#                             ## THIS PART NEEDS TO CHANGE PARTICULARLY FOR REPARAMETRIZATION_STYLE 10
#                             raise NotImplementedError
#                         warm_start_model.fit(self.X_train_, self.l_y_decimal_val_)
#                             print ("************FITTING DONE************ with Elkan c:", warm_start_model.c)
#                             epsilon = 1e-2
#                             warm_gamma = 1 / 2.
#                             warm_lambda = 0.001

        #                     print ("Naive fitting for SPM done")
        #             #                     self.keras_model.sig_layer.alpha.assign(sar_em_model.clf.model.coef_.reshape((-1, 1)))
        #             #                     self.keras_model.sig_layer.beta.assign(sar_em_model.clf.model.intercept_)
        #             #                     self.keras_model.sig_layer.alpha.assign(elkan_model.estimator.coef_.reshape((-1, 1)))
        #             #                     self.keras_model.sig_layer.beta.assign(elkan_model.estimator.intercept_)
        #             #                     self.keras_model.psych_layer.alpha.assign(warm_alpha)
        #             #                     self.keras_model.psych_layer.beta.assign([warm_beta])
        #             #                     c = elkan_model.c
        #             #                     warm_alpha = np.zeros(self.X_train_['sig_input'][0].shape[-1]).reshape((-1, 1))
        #             #                     print ("elkan returned c:", c)
        #             ##################################################################
        #             ##### USING STRAP BEGINNING: IDEA WAS TO WARMSTART FROM ELKAN AND THEN FIX OTHER PARAMETERS OF THE PSYCHOMETRIC FUNCTION EXCEPT GAMMA AND IMPROVE C  USING PSYCHMODEL! DIDN'T WORK, NEEDS MORE WORK.
        #             ###############################################################
        #             #                     bootstrap = False
        #             #                     sig_reg_coef_backup = self.sig_reg_coeff                            
        #             #                     self.sig_reg_coeff = 1. / elkan_model.estimator.C_[0]
        #             #                     self.initialize_model(self.little_input_dict=self.little_input_dict, X_train_['sig_input'])
        #             #                     self.keras_model(self.little_input_dict)
        #             #                     if bootstrap:
        #             #                         self.keras_model.psych_layer.alpha.assign(warm_alpha)
        #             #                         self.keras_model.psych_layer.beta.assign([-20.])
        #             #                         self.keras_model.psych_layer.l_prime.assign([1e-8])
        #             #                         self.keras_model.psych_layer.alpha._trainable = False
        #             #                         self.keras_model.psych_layer.beta._trainable = False
        #             #                         self.keras_model.psych_layer.l_prime._trainable = False
        #             #                         self.keras_model.psych_layer.g_prime.assign([elkan_model.c / (1. - elkan_model.c)])
        #             #                         warm_parameter_reporter = ParameterReporter()
        #             #                         training_ender = EndTraining(monitor='loss', min_delta=1e-3, patience=20, verbose=0, mode='min', baseline=None, restore_best_weights=True)
        #             #                         reduce_lr = ReduceLROnRelativePlateau(monitor='loss', mode='min', factor=0.2, patience=10, min_lr=1e-8, min_delta=1e-3)
        #             #                         dataset_generator = self.create_keras_dataset(self.X_train_, l_y_cat_train_, batch_size=self.batch_size)
        #             #                         warm_model_history_ = self.keras_model.fit(dataset_generator, epochs=100, 
        #             #                                                             use_multiprocessing=self.use_multiprocessing, batch_size=self.batch_size, verbose=self.verbose, callbacks=[warm_parameter_reporter, training_ender, reduce_lr], 
        #             #                                                             workers=workers, validation_data=validation_data_).history
        #             #                         _, _, warm_gamma_prime, warm_lambda_prime =  self.keras_model.psych_layer.get_weights()
        #             #                         warm_a, warm_b, _, _=  self.keras_model.sig_layer.get_weights()
        #             #                         c = (warm_gamma_prime / (1 + warm_gamma_prime + warm_lambda_prime))[0]
        #             #                         print ("c is:", c, "g_prime_init is:", warm_gamma_prime, "and lambda_prime", warm_lambda_prime)
        #             #                     else:

        #             #                         warm_a = elkan_model.estimator.coef_.reshape((-1, 1))
        #             #                         warm_b = elkan_model.estimator.intercept_
        #             # #                         warm_a = sar_em_model.clf.model.coef_.reshape((-1, 1))
        #             # #                         warm_b = sar_em_model.clf.model.intercept_
        #             # #                             warm_a = elkan_model.estimator.coef_.reshape((-1, 1))
        #             # #                             warm_b = elkan_model.estimator.intercept_
        #             #                     self.sig_reg_coeff = sig_reg_coef_backup
        #             ##################################################################
        #             ##### USING STRAP END
        #             ##############################################################
        ...
        #             #                     warm_a = sar_em_cv.best_estimator_.clf.model.coef_.reshape((-1, 1))
        #             #                     warm_b = sar_em_cv.best_estimator_.clf.model.intercept_
        #                     l = (l_y_decimal_train_ / 2).astype(int)
        #                     c = elkan_model.c
        #             #                     c = l.mean() / sar_em_cv.best_estimator_.predict_prob_y_given_x(self.X_train_['sig_input']).mean()
        #             #                     warm_a = elkan_model.estimator.coef_.reshape((-1, 1))
        #             #                     warm_b = elkan_model.estimator.intercept_
        #             #                     if self.is_SPM:
        #                     std = 0.01
        ...
        #             #                         std = 1.
        #             #                     warm_alpha = tf.random.truncated_normal(
        #             #                             shape=[self.X_train_['psych_input'].shape[-1], 1], mean=0.0, stddev=std,
        #             #                             dtype=tf.dtypes.float64) 

        #                     warm_alpha = np.zeros(self.X_train_['psych_input'][0].shape[-1]).reshape((-1, 1))
        #                     warm_alpha = np.random.randn(len(warm_alpha)).reshape(warm_alpha.shape) * std + warm_alpha
        #                     warm_beta = np.log(c / (1 - c))    
        #                     self.initialize_model(self.little_input_dict=self.little_input_dict, sampled_input_features['sig_input']=self.X_train_['sig_input'])
        #             #                     self.keras_model(self.little_input_dict)

        #                     if not self.is_SPM:
        #                         if c > 0.5:
        #                             warm_gamma = self.warm_f_g_prime_init(c)                                                              
        #                             warm_lambda = self.warm_f_l_prime_init(1 - c) 
        #                         else:
        #                             warm_gamma = self.warm_f_g_prime_init(1 - c)                                                              
        #                             warm_lambda = self.warm_f_l_prime_init(c) 
        #                         warm_gamma_prime, warm_lambda_prime = self.keras_model.gamma_lambda_to_g_l_prime_transformer(warm_gamma, warm_lambda)#, reparametrization_style=self.reparametrization_style)
        #                         epsilon = 1e-2
        #                         print ("c is:", c, "warm gamma is:", warm_gamma, "and warm lambda is:", warm_lambda)
        #             #                         self.keras_model.psych_layer.l_prime.assign([warm_lambda_prime])
        #             #                         self.keras_model.psych_layer.g_prime.assign([warm_gamma_prime])
        #                         self.keras_model.psych_layer.l_prime._trainable = True
        #                         self.keras_model.psych_layer.g_prime._trainable = True
        #             #                     alpha_a_dot_product = tf.squeeze(tf.tensordot(warm_a, warm_alpha, axes=(0, 0)))
        #             #                     assert tf.keras.backend.ndim(alpha_a_dot_product) == 0
        #             #                     self.keras_model.psych_layer.alpha.assign(warm_alpha * tf.math.sign(alpha_a_dot_product))
        #             #                     self.keras_model.psych_layer.beta.assign([warm_beta])


#                         self.keras_model.sig_layer.alpha.assign(warm_a)
#                         self.keras_model.sig_layer.beta.assign([warm_b])
        #                     print ("PHASE 1", self.keras_model.sig_layer.alpha.trainable, self.keras_model.sig_layer.beta.trainable, self.keras_model.psych_layer.alpha.trainable, self.keras_model.psych_layer.beta.trainable)
        #                     if self.alternate_training:
        #                         alternating_trials_num = 1
        #                     else:
        #                         alternating_trials_num = 0
        #             #                     self.compile(self.optimizer_name(**{'learning_rate':self.lr}), metrics=self.metrics, model_loss=self.keras_model.model_loss)
        #                     print ("PHASE 2", self.keras_model.sig_layer.alpha.trainable, self.keras_model.sig_layer.beta.trainable, self.keras_model.psych_layer.alpha.trainable, self.keras_model.psych_layer.beta.trainable)
        #                     self.alternating_best_weights = self.keras_model.get_weights()
        #             #                     self.alternating_sig_layer_best_weights = self.keras_model.sig_layer.get_weights(
        #             #                     dataset_generator = self.create_keras_dataset(self.X_train_, l_y_cat_train_, batch_size=self.batch_size)
        #                     if not self.alternate_training:
        #                         try:
        #                             assert alternating_trials_num <= 1
        #                         except AssertionError as e:
        #                             e.args += ('alternating_trials_num is not 1 but it should be since warm_start is true and self.aletnarte_training is False')
        #                             raise                 
        #                     for i in range(alternating_trials_num):
        #                         lr = self.lr
        #             #                         fail_count = 0
        #                         while True:
        ...
        #                             print ("PHASE 3", self.keras_model.sig_layer.alpha.trainable, self.keras_model.sig_layer.beta.trainable, self.keras_model.psych_layer.alpha.trainable, self.keras_model.psych_layer.beta.trainable)
        #                             if lr < 1e-5:
        #                                 break
        ...
        #                             if self.alternate_training:
        #                                 self.keras_model.sig_layer.alpha._trainable = False
        #                                 self.keras_model.sig_layer.beta._trainable = False

        #                             print ("PHASE 4", self.keras_model.sig_layer.alpha.trainable, self.keras_model.sig_layer.beta.trainable, self.keras_model.psych_layer.alpha.trainable, self.keras_model.psych_layer.beta.trainable)
        #                             second_alternate_history, second_alternate_param_reporter, lookback_reduce_lr = self.keras_fit(X_train=self.X_train_, l_y_decimal_train=l_y_decimal_train_, validation_data=validation_data_, lr=lr)
        #             #                             if not(np.isnan(second_alternate_history['loss']).any()) or self.training_failed:
        ...
        #                             lr = lr / 2.
        #             #                             fail_count = 0
        #                         print ("PHASE 5", self.keras_model.sig_layer.alpha.trainable, self.keras_model.sig_layer.beta.trainable, self.keras_model.psych_layer.alpha.trainable, self.keras_model.psych_layer.beta.trainable)

        #                         if self.alternating_best_loss > lookback_reduce_lr.best:
        #                             tf.print("The best weights are updating... prior lowest loss:", self.alternating_best_loss, "curren lowest loss:", lookback_reduce_lr.best, output_stream=sys.stderr)
        #                             self.alternating_best_weights = lookback_reduce_lr.best_weights
        #             #                             self.alternating_psych_layer_best_weights = self.keras_model.psych_layer.get_weights()                    
        #                             self.alternating_best_loss = lookback_reduce_lr.best
        #                             self.alternate_history_list += [second_alternate_history]
        #                             self.alternate_param_reporter_list += [second_alternate_param_reporter]
        #                             self.alternate_descend_epoch_list += [lookback_reduce_lr.descend_epochs]
        #             #                         tf.print("lookback_reduce_lr.best_weights is:", lookback_reduer(ce_lr.best_weights, output_stream=sys.stder
        #                         self.keras_model.sig_layer.alpha._trainable = True
        #                         self.keras_model.sig_layer.beta._trainable = True
        #                         if self.alternate_training:
        #                             lr = self.lr
        #                             fail_count = 0
        #                             while True:
        ...
        #                                 self.keras_model.psych_layer.alpha._trainable = False
        #                                 self.keras_model.psych_layer.beta._trainable = False
        #                                 self.keras_model.psych_layer.g_prime._trainable = False
        #                                 self.keras_model.psych_layer.l_prime._trainable = False
        #                                 print ("PHASE 6", self.keras_model.sig_layer.alpha.trainable, self.keras_model.sig_layer.beta.trainable, self.keras_model.psych_layer.alpha.trainable, self.keras_model.psych_layer.beta.trainable)
        #                                 first_alternate_history, first_alternate_param_reporter, lookback_reduce_lr = self.keras_fit(X_train=self.X_train_, l_y_decimal_train=l_y_demical_train_, validation_data=validation_data_, lr=lr)
        #             #                                 if not(np.isnan(first_alternate_history['loss']).any()) or self.training_failed:
        #             #                                     self.training_failed = False
        #             #                                     break
        #                                 self.initialize_model(self.little_input_dict=self.little_input_dict, sampled_input_features['sig_input']=self.X_train_['sig_input'])
        #             #                                 self.keras_model(self.little_input_dict)
        ...
        #                                 lr = lr / 2.
        #             #                                 fail_count = 0
        #                             print ("PHASE 7", self.keras_model.sig_layer.alpha.trainable, self.keras_model.sig_layer.beta.trainable, self.keras_model.psych_layer.alpha.trainable, self.keras_model.psych_layer.beta.trainable)
        #                             if self.alternating_best_loss > lookback_reduce_lr.best:
        #                                 tf.print("The best weights are updating... prior lowest loss:", self.alternating_best_loss, "curren lowest loss:", lookback_reduce_lr.best, output_stream=sys.stderr)
        #                                 self.alternating_best_weights = lookback_reduce_lr.best_weights
        #             #                                 self.alternating_sig_layer_best_weights = self.keras_model.sig_layer.get_weights()
        #                                 self.alternating_best_loss = lookback_reduce_lr.best
        #             #                                 self.keras_model.set_weights(lookback_reduce_lr.best_weights)
        #                                 self.alternate_history_list += [second_alternate_history]
        #                                 self.alternate_param_reporter_list += [second_alternate_param_reporter]
        #                                 self.alternate_descend_epoch_list += [lookback_reduce_lr.descend_epochs]
        #                     self.keras_model.psych_layer.alpha._trainable = True
        #                     self.keras_model.psych_layer.beta._trainable = True
        #                     if not self.is_SPM:
        #                         self.keras_model.psych_layer.g_prime._trainable = True
        #                         self.keras_model.psych_layer.l_prime._trainable = True
...
#                     self.set_params(**{'psych_beta_init': self.psych_betainit_})
                ...
                try:
                    ...
                    history, parameter_reporter, lookback_reduce_lr = self.keras_fit(X_train=self.X_train_, l_y_decimal_train=self.l_y_decimal_train_, validation_data=validation_data_, lr=lr)
